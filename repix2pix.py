...
parser.add_argument("--display_freq", type=int, default=3000, help="write current training images every display_freq steps")
...
# Model = collections.namedtuple("Model", "outputs, predict_real, predict_fake, cgan_discrim_loss,discrim_loss_per, discrim_grads_and_vars, gen_loss_GAN,histogram_loss, gen_loss_L1,gen_per_loss, gen_grads_and_vars, train")
Model = collections.namedtuple("Model", "outputs, predict_real, predict_fake, cgan_discrim_loss, discrim_grads_and_vars, gen_loss_GAN, gen_loss_L1, gen_grads_and_vars, train")
...
def create_model(inputs, targets):
    ...
    #     weights = [1.0, 2.0, 2.0]
    #     perLoss = 0.0
    #     for i in range(1,len(perceTarget)-2):
    ...
    #         loss = weights[i-1] * tf.reduce_sum(tf.abs(perceTarget[i] - perceOutput[i])) / normalization_factor
    #         perLoss += loss
    #     return perLoss
    ...
    with tf.name_scope("discriminator_loss"):
        ...
        cgan_discrim_loss = tf.reduce_mean(-(tf.log(predict_real[-1] + EPS) + tf.log(1 - predict_fake[-1] + EPS)))
        # discrim_loss =cgan_discrim_loss+discrim_loss_per*a.dis_per_w
        discrim_loss =cgan_discrim_loss
    ...
    # update_losses = ema.apply([discrim_loss_per,cgan_discrim_loss,gen_per_loss, gen_loss_GAN, gen_loss_L1,histogram_loss])
    update_losses = ema.apply([cgan_discrim_loss, gen_loss_GAN, gen_loss_L1])
    ...
    return Model(
        ...
        cgan_discrim_loss=ema.average(cgan_discrim_loss),
...
def main():
    ...
    tf.summary.scalar("cgan_discrim_loss", model.cgan_discrim_loss)
        ...
        if a.mode == "test":
            ...
            for step in range(max_steps):
                ...
                if should(a.progress_freq):
                    fetches["cgan_discrim_loss"] = model.cgan_discrim_loss
                ...
                if should(a.progress_freq):
                    ...
                    print("cgan_discrim_loss", results["cgan_discrim_loss"])
