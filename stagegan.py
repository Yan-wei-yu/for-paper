...
#--display_freq:類型：int默認值：500說明：每display_freq步寫當前訓練圖像。用途：設置圖像顯示的頻率。
parser.add_argument("--display_freq", type=int, default=500,
                    help="write current training images every display_freq steps")
# --save_freq:類型：int默認值：1000說明：每save_freq步保存模型（設為0則禁用）。用途：設置模型保存的頻率。
...
def create_model(inputs, condition1, condition2, targets):
    ...
    with tf.name_scope("generator_loss"):
        ...
        # 中央溝損失：30000步後加入
        gen_loss_CenSul = tf.cond(
            tf.greater_equal(tf.train.get_or_create_global_step(), 30000),
            lambda: tf.reduce_mean(tf.abs(cenSulTarget - cenSulOutput)) * a.cenSul_weight,
        ...
        histogram_loss = tf.cond(
            ...
            lambda: tf.reduce_mean(
                tf.divide(
                    ...
                    tf.maximum(1.0, tf.cast(hist_real, tf.float32))
                )
            ) * a.hist_weight,
            lambda: tf.constant(0.0)
        )
...
def main():
    ...
    # a.checkpoint = "D://Users//user//Desktop//weiyundontdelete//GANdata//trainingdepth//DAISdepth//alldata//GANL1pergronetTESThis250crop//"
    # a.mode = "export"
    # a.output_dir = "D://Users//user//Desktop//weiyundontdelete//GANdata//trainingdepth//DAISdepth//alldata//GANL1pergronetTESThiscropV1export//"
    # a.which_direction = "BtoA"
    ...
    # 動態損失
    tf.summary.scalar("generator_loss_cenSul", tf.cond(
        tf.greater_equal(tf.train.get_or_create_global_step(), 30000),
        lambda: model.gen_loss_CenSul,
        lambda: 0.0
    ))
        ...
        if a.mode == "test":
            ...
            # 條件：當頻率大於 0 且當前步數符合頻率條件，或者是最後一步。
                ...
                if should(a.summary_freq):
                    ...
                    summary = sess.run(loss_summaries)
                    sv.summary_writer.add_summary(summary, results["global_step"])
                # 保存生成的顯示圖片，並更新索引文件。
                if should(a.display_freq):
